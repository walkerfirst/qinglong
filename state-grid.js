...
async function getMonthElecQuantity(userIndex) {
  console.log(`⏳ 获取户号${userIndex + 1}的月用电量...`);
  const userInfo = bindInfo.powerUserList[userIndex];
  const accountInfo = bizrt.userInfo && bizrt.userInfo.length > 0 ? bizrt.userInfo[0] : {};
  ...
  try {
    ...
    const requestData = {
      ...
      data: {
        params1: {
          ...
          quInfo: { userId: accountInfo.userId },
          token: bizrt.token,
        },
        params3: {
          data: {
            acctId: accountInfo.userId,
            consNo: userInfo.consNo_dst,
            consType: userInfo.constType === '02' ? '02' : '01',
            orgNo: userInfo.orgNo,
            proCode: userInfo.proNo || userInfo.provinceId,
            provinceCode: userInfo.proNo || userInfo.provinceId,
            queryYear: currentYear,
            serialNo: '',
            srvCode: '',
            userName: accountInfo.nickname || accountInfo.loginAccount,
          ...
          target: userInfo.proNo || userInfo.provinceId,
        },
        params4: '010102',
      },
    };
    ...
    // 发送请求获取当年数据
    console.log('发送当年数据请求...');
    const response = await request(requestData);
    
    // 处理响应数据
    if (!response || !response.mothEleList || !Array.isArray(response.mothEleList)) {
      throw new Error('获取月用电量数据失败：响应格式不正确');
    }
    
    console.log(`获取到 ${response.mothEleList.length} 个月份数据`);
    
    // 格式化数据
    const formattedData = response.mothEleList.map(month => ({
      month: month.month,
      monthEleNum: month.elecNum || month.monthEleNum || '0',
      monthEleCost: month.elecCost || month.monthEleCost || '0'
    }));
    ...
    // 计算总用电量和总电费
    const totalEleNum = formattedData.reduce((sum, month) => sum + parseFloat(month.monthEleNum || 0), 0).toFixed(2);
    const totalEleCost = formattedData.reduce((sum, month) => sum + parseFloat(month.monthEleCost || 0), 0).toFixed(2);
    
    const result = {
      mothEleList: formattedData,
      dataInfo: {
        totalEleNum: totalEleNum,
        totalEleCost: totalEleCost,
        year: currentYear
      }
    };
    
    //console.log('处理后的月用电量数据:', JSON.stringify(result, null, 2));
    log.info(`✅ 获取户号${userIndex + 1}的月用电量成功`);
    
    return result;
    
  } catch (error) {
    const errorMsg = `获取户号${userIndex + 1}的月用电量失败: ${error.message || error}`;
    ...
    console.log(`🔚 获取户号${userIndex + 1}的月用电量结束`);
  }
}
...
function formatDate(dateStr) {
  ...
  // 返回格式化的日期字符串，包含星期
  return day ? `${year}-${month}-${day}(${dayOfWeek})` : `${year}-${month}`;
}
// 简化版输出信息函数
async function sendMsg(title, eleBill = {}, dayList = [], monthElecQuantity = {}) {
  try {
    // 确保 eleBill 是对象
    eleBill = eleBill || {};
    
    // 处理日用电量数据
    const validDayList = (Array.isArray(dayList) ? dayList : [])
      .filter(val => val && val.dayElePq != '-')
      .map(val => ({
        ...val,
    ...
    // 处理月用电量数据
    const monthData = monthElecQuantity || {};
    const monthList = (Array.isArray(monthData.mothEleList) ? monthData.mothEleList : [])
    ...
    // 确保 dataInfo 存在
    const dataInfo = monthData.dataInfo || {};
    ...
    // 添加账户信息
    content += '【账户信息】\n';
    content += `户号: ${eleBill.consNo || '无'}\n`;
    content += `户名: ${eleBill.consName || '无'}\n`;
    content += `用电地址: ${eleBill.elecAddr || '无'}\n\n`;
    
    // 添加电费余额信息
    const isOwe = Number(eleBill?.historyOwe || '0') > 0 || Number(eleBill?.sumMoney || '0') < 0;
    let balance = Math.abs(eleBill?.sumMoney || '0');
    balance = isOwe ? `-${balance}` : balance;
    
    content += '【电费余额】\n';
    content += `当前余额: ${balance} 元\n`;
    
    if (eleBill.dayNum) {
      content += `预计可用: ${eleBill.dayNum} 天\n`;
    } else if (eleBill.prepayBal) {
    ...
    if (validDayList.length > 0) {
      ...
      validDayList.forEach(day => {
        content += `${day.day}: ${day.dayElePq} 度\n`;
      });
    }
    ...
    try {
      ...
      console.log('电费信息已通过通知发送');
    } catch (notifyError) {
      console.error('发送通知失败:', notifyError);
    }
  } catch (error) {
    console.error('输出电费信息时出错:', error);
  }
}
...
(async () => {
  ...
  // 移除过滤逻辑，保留所有户号
  // 只保留有效的户号（有consNo_dst的）
  bindInfo.powerUserList = bindInfo.powerUserList.filter(
    e => e.consNo_dst && e.consNo_dst.trim() !== ''
  );
  
  console.log(`找到 ${bindInfo.powerUserList.length} 个有效户号`);
  bindInfo.powerUserList.forEach((user, index) => {
    console.log(`户号 ${index + 1}: ${user.consNo_dst} (${user.consName_dst || '无户名'})`);
  });
  // 获取要查询的户号索引，如果指定了WSGW_ACCOUNT_INDEX则只查询指定户号，否则查询所有户号
  let accountIndex = null;
  if (process.env.WSGW_ACCOUNT_INDEX) {
    accountIndex = parseInt(process.env.WSGW_ACCOUNT_INDEX, 10) - 1;
    if (isNaN(accountIndex) || accountIndex < 0 || accountIndex >= bindInfo.powerUserList.length) {
      console.log(`指定的户号索引 ${process.env.WSGW_ACCOUNT_INDEX} 无效，将查询所有户号`);
      accountIndex = null;
    }
  }
  ...
  if (accountIndex !== null) {
    userIndices = [accountIndex];
  } else {
    userIndices = Array.from({ length: bindInfo.powerUserList.length }, (_, i) => i);
  }
  
  console.log(`\n=== 开始查询${userIndices.length}个户号的电费信息 ===`);
  
  for (const userIndex of userIndices) {
    const userInfo = bindInfo.powerUserList[userIndex];
    console.log(`\n📋 正在处理户号 ${userIndex + 1}/${bindInfo.powerUserList.length}: ${userInfo.consNo_dst || '未知户号'}`);
    
    try {
      // 获取电费信息
      await getElcFee(userIndex);
      
      // 获取日用电量
      await getDayElecQuantity(userIndex);
      ...
      try {
        const monthDataResult = await getMonthElecQuantity(userIndex, 12);
        if (monthDataResult) {
          monthData = monthDataResult;
        }
      } catch (error) {
        console.error('获取月用电量时出错:', error);
        // 继续使用初始化的空数据
      }
      ...
      const userBill = {
        ...(eleBill || {}),
        consNo: userInfo.consNo_dst || '',
        consName: userInfo.consName_dst || '',
        elecAddr: userInfo.elecAddr_dst || '',
        orgName: userInfo.orgName || ''
      };
      
      // 确保eleBill中的必要字段存在
      userBill.historyOwe = userBill.historyOwe || '0';
      userBill.sumMoney = userBill.sumMoney || '0';
      userBill.dayNum = userBill.dayNum || '';
      userBill.prepayBal = userBill.prepayBal || '0';
      
      // 获取日用电量数据
      const { sevenEleList: dayList = [] } = dayElecQuantity || {};
      
      // 输出当前户号的信息
      await sendMsg(SCRIPTNAME, userBill, dayList, monthData);
      
      // 添加间隔，避免请求过于频繁
      if (userIndex < userIndices.length - 1) {
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    } catch (error) {
      console.error(`处理户号 ${userIndex + 1} 时出错:`, error);
      // 继续处理下一个户号
      continue;
    }
  }
